                    ...
                    <div class="page-title-box">
                        <div class="page-title-right">
                            <ol class="breadcrumb m-0">
                                <h4>
                                    <a href="{{ route('employee.attend.list') }} " class="btn btn-primary float-sm-right"> <i class="fa fa-list px-1"> </i>Employee Attendance List</a>
                                </h4>
                            </ol>
                        </div>
                    </div>
                        ...
                        <div class="card-body">
                            <form action=" {{ route('employee.attend.update') }}" method="post" id="myForm">
                                ...
                                <div class="form-group col-md-4">
                                    <label for="date" class="control-label">Attendance Date (M/D/Y)</label>
                                    <input type="date" name="date" id="date" class="checkdate form-control form-control-sm singledatepicker" placeholder="Attendance Date" autocomplete="off" value="{{ $dateWiseData[0]['date'] }}">
                                </div>
                                <table class="table sm table-bordered table-striped dt-responsive" style="width: 100%">
                                    ...
                                    <tr>
                                        <th class="text-center btn present_all" style="display: table-cell;background-color:lavenderblush">Present</th>
                                        <th class="text-center btn leave_all" style="display: table-cell;background-color:lavenderblush">Leave</th>
                                        <th class="text-center btn absent_all" style="display: table-cell;background-color:lavenderblush">Absent</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                        ...
                                        <tr class="text-center">
                                            ...
                                            <td colspan="3">
                                                <div class="switch-toggle switch-3 switch-candy">
                                                    <input class="present" id="present{{$key}}" name="attend_status{{$key}}" value="present" {{($item->attend_status == "present") ? 'checked' : ''}} type="radio" checked="checked">
                                                    <label for="present{{$key}}">Present</label>

                                                    <input class="leave" id="leave{{$key}}" name="attend_status{{$key}}" value="Leave" {{($item->attend_status == "Leave") ? 'checked' : ''}} type="radio">
                                                    <label for="leave{{$key}}">Leave</label>

                                                    <input class="absent" id="absent{{$key}}" name="attend_status{{$key}}" value="Absent" {{($item->attend_status == "Absent") ? 'checked' : ''}} type="radio">
